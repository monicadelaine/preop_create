...
START = chr(128)    # already converted to bytes...serial
BAUD = chr(129)     # + 1 byte
...
#======================== Starting up and Shutting Down================    
    def __init__(self, PORT, startingMode=SAFE_MODE, sim_mode = False):
        ...
        if type(PORT) == type('string'):
                ...
                self.ser.connect ((self._address,self._port))
    ...
    def send(self, bytes1):
        self.ser.send( (bytes(bytes1)) )

    def read(self, bytes):
        message = ""
        message = self.ser.recv( bytes )
        return str(message);
